...
public:
	...
	void update(Tree& tree) override {
		...
		splitters.resize(trees::max_idx_at_depth(tree.get_max_depth()), MSESplitter(this->min_samples_leaf));
		...
		for (size_t curr_depth = 0; curr_depth < tree.get_max_depth(); curr_depth++) {
			...
			for (auto parent : nodes_old) {
				...
				if (split.succesful) {
					if (split.r_n >= this->min_samples_split) nodes.push_back(trees::right_child(parent));
					if (split.l_n >= this->min_samples_split) nodes.push_back(trees::left_child(parent));
				}
			}
		}
